...
void q4 () {
	...
	std::thread out_thread = std::thread([&cnt, &_out, &sum_sch](){
		cnt = clear_pipe (_out, &sum_sch, true);
	});
...
void q6 () {
	...
	Attribute final_att[] = {DA, s_nationkey};
	Schema sum_sch ("sum_sch", 2, final_att);
	int cnt;
	std::thread out_thread = std::thread([&cnt, &_out, &sum_sch](){
		cnt = clear_pipe (_out, &sum_sch, true);
	});
